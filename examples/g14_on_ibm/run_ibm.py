...
def get_cli_args() -> argparse.Namespace:
    ...
    parser.add_argument('--jobs', default=None,
                        help='Path to file containing old job ids to retrieve')
...
def run(account: str,
    ...
    if jobs is not None:
        ...
        tasks = [
            IBMTask(None, None, None, account, shots=shots, job_id=id)
            for id in job_ids]
    else:
...
class IBMTask:
    ...
    def run(self):
        ...
        # Retrieve job from the cloud given its id
            ...
            print(f'Retrieved job {self.job_id} for backend {self.backend.name}', flush=True)
    ...
    def _make_df(self, questions: Sequence[tuple], count_results: Sequence[Counts]) -> pd.DataFrame:
